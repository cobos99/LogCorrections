...
def get_xxz_numerical_max_amps(N_arr, delta_arr, global_neg, return_energy=False, return_max_amp_ind=False, return_cnum=False, comparison_tol=1e-8, print_mode=False, save=True):
    ...
    for i, N in enumerate(N_arr):
        gs_ks = [0] if (global_neg or N % 2 == 0) else [(N - N // 2 - 1)//2 + (N - N // 2 - 1) % 2, N - (N - N // 2 - 1)//2 - (N - N // 2 - 1) % 2]
        bases = None
...
def get_xxz_numerical_shannon(N_arr, delta_arr, global_neg, return_energy=False, print_mode=False, save=True):
    N_arr = np.asarray(N_arr)
    if N_arr.ndim == 0:
        N_arr = N_arr[None]
    delta_arr = np.asarray(delta_arr)
